...
def main():
    ...
    model = GhostNetV2_FER(num_classes=Config.NUM_CLASSES, dropout=0.3).to(Config.DEVICE)
    print(f"CNN (GhostNetV2) OK | Параметры: {sum(p.numel() for p in model.parameters()):,}")
    ...
    best_acc = 0.0
    best_path = os.path.join(Config.MODEL_SAVE_PATH, 'best_model_cnn.pth')
    start = time.time()
    ...
    for epoch in range(1, Config.NUM_EPOCHS + 1):
        ...
        if val_acc > best_acc:
            ...
            print(f'Сохранена лучшая CNN-модель (acc={best_acc:.2f}%)')
